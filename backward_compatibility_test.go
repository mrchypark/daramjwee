...
func (f *backwardCompatibilityFetcher) Fetch(ctx context.Context, metadata *Metadata) (*FetchResult, error) {
	return &FetchResult{
...
func TestBackwardCompatibilityConfiguration(t *testing.T) {
	...
	t.Run("WithHotStore only - legacy vs new", func(t *testing.T) {
		...
		legacyCache, err := New(logger, WithHotStore(hotStore))
		require.NoError(t, err)
		defer legacyCache.Close()

		// New configuration equivalent
		newCache, err := New(logger, WithStores(hotStore))
		require.NoError(t, err)
		defer newCache.Close()

		// Both should have identical structure
		legacyDaramjwee := legacyCache.(*DaramjweeCache)
		newDaramjwee := newCache.(*DaramjweeCache)
	...
	t.Run("WithHotStore + WithColdStore - legacy vs new", func(t *testing.T) {
		...
		// Legacy configuration
		legacyCache, err := New(logger, WithHotStore(hotStore), WithColdStore(coldStore))
		require.NoError(t, err)
		defer legacyCache.Close()

		// New configuration equivalent
		newCache, err := New(logger, WithStores(hotStore, coldStore))
		require.NoError(t, err)
		defer newCache.Close()
	...
	t.Run("WithColdStore without WithHotStore should fail", func(t *testing.T) {
		...
		cache, err := New(logger, WithColdStore(coldStore))
		require.Error(t, err)
		require.Nil(t, cache)
...
func TestBackwardCompatibilityBehavior(t *testing.T) {
	...
	testCases := []struct {
		name           string
		setupLegacy    func() (Cache, error)
		setupNew       func() (Cache, error)
		expectedStores int
	}{
		{
			name: "single tier behavior",
			setupLegacy: func() (Cache, error) {
				return New(logger, WithHotStore(newMockStore()))
			},
			setupNew: func() (Cache, error) {
				return New(logger, WithStores(newMockStore()))
			},
			expectedStores: 1,
		},
		{
			...
			setupLegacy: func() (Cache, error) {
				return New(logger, WithHotStore(newMockStore()), WithColdStore(newMockStore()))
			},
			setupNew: func() (Cache, error) {
				return New(logger, WithStores(newMockStore(), newMockStore()))
			},
			expectedStores: 2,
		},
	}
...
func TestLegacyConfigurationMigration(t *testing.T) {
	...
	t.Run("migrate from WithHotStore only", func(t *testing.T) {
		...
		migratedCache, err := New(logger, WithStores(hotStore))
		require.NoError(t, err)
	...
	t.Run("migrate from WithHotStore + WithColdStore", func(t *testing.T) {
		...
		// Migrated configuration
		migratedCache, err := New(logger, WithStores(hotStore, coldStore))
		require.NoError(t, err)
		defer migratedCache.Close()
...
func TestLegacyConfigurationConflictPrevention(t *testing.T) {
	...
	t.Run("WithHotStore conflicts with WithStores", func(t *testing.T) {
		cache, err := New(logger, WithHotStore(newMockStore()), WithStores(newMockStore()))
		require.Error(t, err)
		require.Nil(t, cache)
	...
	t.Run("WithColdStore conflicts with WithStores", func(t *testing.T) {
		cache, err := New(logger, WithColdStore(newMockStore()), WithStores(newMockStore()))
		require.Error(t, err)
		require.Nil(t, cache)
	...
	t.Run("WithStores conflicts with existing WithHotStore", func(t *testing.T) {
		cache, err := New(logger, WithStores(newMockStore()), WithHotStore(newMockStore()))
		require.Error(t, err)
		require.Nil(t, cache)
...
func TestLegacyConfigurationErrorMessages(t *testing.T) {
	...
	t.Run("nil hot store error message", func(t *testing.T) {
		cache, err := New(logger, WithHotStore(nil))
		require.Error(t, err)
		require.Nil(t, cache)
	...
	t.Run("missing hot store error message", func(t *testing.T) {
		cache, err := New(logger, WithColdStore(newMockStore()))
		require.Error(t, err)
		require.Nil(t, cache)
		...
		b.Run(dataSize.name, func(b *testing.B) {
			b.Run("SingleTier", func(b *testing.B) {
				b.Run("Legacy_WithHotStore", func(b *testing.B) {
					benchmarkCacheOperations(b, logger, dataSize.size, func() (Cache, error) {
						return New(logger, WithHotStore(newMockStore()))
					})
				})
...
func benchmarkCacheOperations(b *testing.B, logger log.Logger, dataSize int, setupCache func() (Cache, error)) {
	...
	for _, concurrency := range concurrencyLevels {
		b.Run(fmt.Sprintf("Concurrency_%d", concurrency), func(b *testing.B) {
			b.Run("Legacy_TwoTier", func(b *testing.B) {
				benchmarkBackwardCompatibilityConcurrentOperations(b, logger, testData, concurrency, func() (Cache, error) {
					return New(logger, WithHotStore(newMockStore()), WithColdStore(newMockStore()))
				})
			})
...
func benchmarkBackwardCompatibilityConcurrentOperations(b *testing.B, logger log.Logger, testData []byte, concurrency int, setupCache func() (Cache, error)) {
	cache, err := setupCache()
	if err != nil {
		b.Fatalf("Failed to setup cache: %v", err)
	}

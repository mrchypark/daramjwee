...
func TestIfNoneMatchMatchesCacheTag(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "quoted comma match", ifNoneMatch: "\"rev,42\"", cacheTag: "rev,42", want: true},
		{name: "wildcard", ifNoneMatch: "*", cacheTag: "cache-v1", want: true},
		{name: "wildcard matches existing entry without cache tag", ifNoneMatch: "*", cacheTag: "", want: true},

...
func TestErrorHandlingAndFallbacks(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			pool, err := daramjwee.NewAdaptiveBufferPool(tc.config, logger)
...
func (cv *ConfigurationValidator) ValidateConfiguration(config daramjwee.BufferPoolConfig) error {
	if !config.Enabled {
...
func (cv *ConfigurationValidator) GenerateRecommendations(config daramjwee.BufferPoolConfig) []ConfigurationRecommendation {
...
func (cv *ConfigurationValidator) DetectMisconfigurations(config daramjwee.BufferPoolConfig) []string {
	var issues []string

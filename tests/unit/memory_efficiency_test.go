...
func TestMemoryUsagePatterns(t *testing.T) {
	...
	testCases := []struct {
		...
		{"Small_Pooled", 16 * 1024, daramjwee.StrategyPooled, 1000},
		{"Medium_Pooled", 128 * 1024, daramjwee.StrategyPooled, 500},
...
func TestMemoryEfficiencyComparison(t *testing.T) {
	...
	adaptiveConfig := daramjwee.BufferPoolConfig{
		Enabled:                  true,
		DefaultBufferSize:        32 * 1024,
		MaxBufferSize:            1024 * 1024,
		MinBufferSize:            1 * 1024,
	...
	adaptivePool, err := daramjwee.NewAdaptiveBufferPool(adaptiveConfig, logger)
	require.NoError(t, err)
	...
	disabledPool, err := daramjwee.NewAdaptiveBufferPool(disabledConfig, logger)
	require.NoError(t, err)

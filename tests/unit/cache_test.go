...
// mockFetcher is a mock implementation of the daramjwee.Fetcher interface for testing purposes.
...
// mockStore is a mock implementation of the daramjwee.Store interface for testing purposes.
...
func (s *mockStore) setNegativeEntry(key string, metadata *daramjwee.Metadata) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (f *deterministicFetcher) Fetch(ctx context.Context, oldMetadata *daramjwee.Metadata) (*daramjwee.FetchResult, error) {
...
func TestCache_Get_ColdHit(t *testing.T) {
	...
	cold.setData(key, content, &daramjwee.Metadata{ETag: etag, CachedAt: time.Now().Add(-1 * time.Hour)})
...
func TestCache_StaleHit_ServesStaleWhileRefreshing(t *testing.T) {
	cache, hot, _ := setupCache(t, daramjwee.WithCache(1*time.Minute))
	ctx := context.Background()
	key, staleContent, freshContent := "stale-key", "stale data", "fresh data"
	hot.setData(key, staleContent, &daramjwee.Metadata{ETag: "v1", CachedAt: time.Now().Add(-2 * time.Minute)})
...
func TestCacheableNotFound(t *testing.T) {
	cache, hot, _ := setupCache(t, daramjwee.WithNegativeCache(5*time.Minute))
	ctx := context.Background()
	key := "negative-key"
...
func TestCache_StaleHit(t *testing.T) {
	cache, hot, _ := setupCache(t, daramjwee.WithNegativeCache(1*time.Minute))
	ctx := context.Background()
	key, freshContent := "stale-negative-key", "now it exists"
	hot.setNegativeEntry(key, &daramjwee.Metadata{CachedAt: time.Now().Add(-2 * time.Minute)})
...
func TestCache_Get_ContextCancellation(t *testing.T) {
	cache, _, _ := setupCache(t)
	fetcher := &mockFetcher{content: "some data", fetchDelay: 2 * time.Millisecond}
...
func TestCache_Set_Directly(t *testing.T) {
	...
	writer, err := cache.Set(ctx, key, &daramjwee.Metadata{ETag: etag})
	require.NoError(t, err)
...
func TestCache_Delete(t *testing.T) {
	cache, hot, cold := setupCache(t)
	key := "delete-key"
	hot.setData(key, "hot", &daramjwee.Metadata{})
	cold.setData(key, "cold", &daramjwee.Metadata{})
...
func TestCache_Close(t *testing.T) {
	cache, hot, _ := setupCache(t, daramjwee.WithCache(1*time.Minute))
	key := "key-after-close"
	hot.setData(key, "data", &daramjwee.Metadata{CachedAt: time.Now().Add(-2 * time.Minute)})
...
func TestCache_Get_NotModified_ButEvictedRace_Deterministic(t *testing.T) {
	...
	key := "race-key"
	hot.mu.Lock()
	hot.meta[key] = &daramjwee.Metadata{ETag: "v1", CachedAt: time.Now()}
	hot.mu.Unlock()

	fetcher := newDeterministicFetcher("", "v1", daramjwee.ErrNotModified)
...
func TestCache_Concurrent_GetAndDelete(t *testing.T) {
	cache, hot, _ := setupCache(t)
	key := "get-delete-key"
	hot.setData(key, "some content", &daramjwee.Metadata{})
	...
	go func() {
		defer wg.Done()
		fetcherForGet := &mockFetcher{err: daramjwee.ErrNotModified, etag: "v1"}
	...
	assert.ErrorIs(t, err, daramjwee.ErrNotFound, "Item should be deleted after all operations")
}
...
func TestCache_ReturnsError_AfterClose(t *testing.T) {
	...
	_, setErr := cache.Set(ctx, key, &daramjwee.Metadata{})
	...
	if dc, ok := cache.(*daramjwee.DaramjweeCache); ok {
		if dc.isClosed.Load() {
		}
	}
}
...
func TestCache_Get_ColdHit_PromotionFails(t *testing.T) {
	...
	cold.setData(key, content, &daramjwee.Metadata{ETag: etag})

	cache, err := daramjwee.New(log.NewNopLogger(), daramjwee.WithHotStore(hot), daramjwee.WithColdStore(cold))
	require.NoError(t, err)
	defer cache.Close()
...
func TestCache_FreshForZero_AlwaysTriggersRefresh(t *testing.T) {
	t.Run("PositiveCacheWithFreshForZero", func(t *testing.T) {
		cache, hot, _ := setupCache(t, daramjwee.WithCache(0))
		ctx := context.Background()
		...
		hot.setData(key, initialContent, &daramjwee.Metadata{ETag: "v1", CachedAt: time.Now()})
		...
		time.Sleep(5 * time.Millisecond)
		assert.Equal(t, 1, fetcher.getFetchCount(), "daramjwee.Fetcher should be called once after the first Get()")
		...
		time.Sleep(5 * time.Millisecond)
		assert.Equal(t, 2, fetcher.getFetchCount(), "daramjwee.Fetcher should be called twice after the second Get()")
	})

	t.Run("NegativeCacheWithFreshForZero", func(t *testing.T) {
		cache, hot, _ := setupCache(t, daramjwee.WithNegativeCache(daramjwee.WithLargeObjectOptimization))
		ctx := context.Background()
		key := "negative-fresh-for-zero-key"

		fetcher := &mockFetcher{err: ErrCacheableNotFound}

		hot.setNegativeEntry(key, &daramjwee.Metadata{CachedAt: time.Now()})

		_, err := cache.Get(ctx, key, fetcher)
		require.ErrorIs(t, err, daramjwee.ErrNotFound)

		time.Sleep(5 * time.Millisecond)
		assert.Equal(t, 1, fetcher.getFetchCount(), "daramjwee.Fetcher should be called once after the first Get() for negative cache")

		_, err = cache.Get(ctx, key, fetcher)
		require.ErrorIs(t, err, daramjwee.ErrNotFound)

		time.Sleep(5 * time.Millisecond)
		assert.Equal(t, 2, fetcher.getFetchCount(), "daramjwee.Fetcher should be called twice after the second Get() for negative cache")
	})
}
...
func BenchmarkCache_Get_HotHit(b *testing.B) {
	...
	hot.setData(key, content, &daramjwee.Metadata{})
...
func BenchmarkCache_Get_ColdHit(b *testing.B) {
	...
	cold.setData(key, content, &daramjwee.Metadata{})
	fetcher := &mockFetcher{}

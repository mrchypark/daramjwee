...
func TestNTierIntegration(t *testing.T) {
	...
	t.Run("3-tier setup (memory → file → cloud)", func(t *testing.T) {
		...
		cache, err := daramjwee.New(logger, WithStores(memoryStore, fileStore, cloudStore))
		...
		// Test 1: daramjwee.Cache miss - should fetch from origin and store in primary tier
		...
		assert.Equal(t, int32(1), atomic.LoadInt32(&fetcher.called), "daramjwee.Fetcher should be called for cache miss")
		...
		// Test 2: daramjwee.Cache hit from primary tier - should not call fetcher
		...
		assert.Equal(t, int32(0), atomic.LoadInt32(&fetcher2.called), "daramjwee.Fetcher should not be called for primary tier hit")
	})

	t.Run("single-tier configuration", func(t *testing.T) {
		singleStore := newMockStore()

		cache, err := daramjwee.New(logger, WithStores(singleStore))
		require.NoError(t, err)
		defer cache.Close()
	...
	t.Run("promotion behavior across multiple tiers", func(t *testing.T) {
		...
		tier2.meta["promotion-key"] = &daramjwee.Metadata{
			ETag:     "tier2-etag",
			CachedAt: time.Now(),
		}
	...
	t.Run("failure scenarios and recovery", func(t *testing.T) {
		...
		cache, err := daramjwee.New(logger, WithStores(workingStore, failingStore, recoveryStore))
		require.NoError(t, err)
		defer cache.Close()
...
// integrationFetcher implements daramjwee.Fetcher interface for integration testing
...
func (f *integrationFetcher) Fetch(ctx context.Context, metadata *daramjwee.Metadata) (*daramjwee.FetchResult, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
...
func (s *integrationFailingStore) GetStream(ctx context.Context, key string) (io.ReadCloser, *daramjwee.Metadata, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()

	if s.shouldFailGet {
		return nil, nil, errors.daramjwee.New("simulated get failure")
	}
...
func (s *integrationFailingStore) SetWithWriter(ctx context.Context, key string, metadata *daramjwee.Metadata) (io.WriteCloser, error) {
	if s.shouldFailSet {
		return nil, errors.daramjwee.New("simulated set failure")
	}
...
func (s *integrationFailingStore) Delete(ctx context.Context, key string) error {
	...
	if s.shouldFailDelete {
		return errors.daramjwee.New("simulated delete failure")
	}
...
func (s *integrationFailingStore) Stat(ctx context.Context, key string) (*daramjwee.Metadata, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (w *integrationMockWriter) Close() error {
	...
	if w.store.meta == nil {
		w.store.meta = make(map[string]*daramjwee.Metadata)
	}

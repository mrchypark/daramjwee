...
func BenchmarkAdaptiveBufferPool_AllSizeCategories(b *testing.B) {
	...
	testSizes := []struct {
		...
		{"Small_1KB", 1 * 1024, daramjwee.SizeCategorySmall},
		{"Small_4KB", 4 * 1024, daramjwee.SizeCategorySmall},
		{"Small_16KB", 16 * 1024, daramjwee.SizeCategorySmall},
		{"Medium_32KB", 32 * 1024, daramjwee.SizeCategoryMedium},
		{"Medium_64KB", 64 * 1024, daramjwee.SizeCategoryMedium},
		{"Medium_128KB", 128 * 1024, daramjwee.SizeCategoryMedium},
		{"Large_256KB", 256 * 1024, daramjwee.SizeCategoryLarge},
		{"Large_512KB", 512 * 1024, daramjwee.SizeCategoryLarge},
...
func (pts *PerformanceTestSuite) benchmarkBufferPoolOperations(b *testing.B, size int, category daramjwee.ObjectSizeCategory) {
	...
	pool, err := daramjwee.NewAdaptiveBufferPool(config, pts.logger)
	...
	// daramjwee.Store baseline or check for regression
...
func BenchmarkStrategyComparison(b *testing.B) {
	...
	testCases := []struct {
		...
		{"Pooled_32KB", 32 * 1024, daramjwee.StrategyPooled},
...
func TestPerformanceRegressionDetection(t *testing.T) {
	...
	baseline := PerformanceBaseline{
		ObjectSize:     32 * 1024,
		Strategy:       daramjwee.StrategyPooled,
...
func TestMemoryAllocationPatterns(t *testing.T) {
	...
	for _, size := range testSizes {
		t.Run(fmt.Sprintf("Size_%dKB", size/1024), func(t *testing.T) {
			...
			// Note: daramjwee.AdaptiveBufferPool has more complexity than simple pools

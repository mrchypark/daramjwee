...
func benchmarkBackgroundRefresh(b *testing.B, dataSize int, useBufferPool bool) {
	...
	for i := 0; i < b.N; i++ {
		...
		keys[i] = key
		hot.setData(key, string(testData), &daramjwee.Metadata{
			ETag:     "old-etag",
...
func benchmarkConcurrentOperations(b *testing.B, dataSize int, useBufferPool bool) {
	...
	for i := 0; i < numKeys; i++ {
		...
		if i%3 == 0 {
			// Hot cache hit
			hot.setData(key, string(testData), &daramjwee.Metadata{ETag: "hot-etag"})
		} else if i%3 == 1 {
			// Cold cache hit
			cold.setData(key, string(testData), &daramjwee.Metadata{ETag: "cold-etag"})
		}
	...
	b.RunParallel(func(pb *testing.PB) {
		...
		for pb.Next() {
			...
			case 2: // Set operation (12.5% of traffic)
				writer, err := cache.Set(context.Background(), key, &daramjwee.Metadata{ETag: "set-etag"})
				if err != nil {
...
func benchmarkMemoryAllocation(b *testing.B, dataSize, poolSize int, useBufferPool bool) {
	...
	var opts []daramjwee.Option
	if useBufferPool {
		opts = append(opts, daramjwee.WithBufferPool(true, poolSize))
	}
	...
	if useBufferPool {
		if dc, ok := cache.(*daramjwee.DaramjweeCache); ok && dc.daramjwee.BufferPool != nil {
			if bp, ok := dc.daramjwee.BufferPool.(*DefaultBufferPool); ok {
				stats := bp.GetStats()

	...
	for _, ds := range dataSizes {
		b.Run(ds.name, func(b *testing.B) {
			...
			b.Run("2-tier-primary-hits", func(b *testing.B) {
				...
				cache, err := daramjwee.New(logger, WithStores(hotStore, coldStore))
				if err != nil {
				...
				writer, err := cache.Set(context.Background(), "bench-key", &daramjwee.Metadata{ETag: "bench-etag", CachedAt: time.Now()})
				if err != nil {
			...
			b.Run("3-tier-primary-hits", func(b *testing.B) {
				...
				memoryStore.meta["bench-key"] = &daramjwee.Metadata{
					ETag:     "bench-etag",
					CachedAt: time.Now(),
				}
			...
			b.Run("5-tier-primary-hits", func(b *testing.B) {
				...
				stores := make([]daramjwee.Store, 5)
				for i := range 5 {
					stores[i] = newMockStore()
				}

				cache, err := daramjwee.New(logger, WithStores(stores...))
				if err != nil {
					b.Fatalf("Failed to create cache: %v", err)
				}
				defer cache.Close()
				...
				stores[0].(*mockStore).meta["bench-key"] = &daramjwee.Metadata{
					ETag:     "bench-etag",
					CachedAt: time.Now(),
				}
	...
	for _, ds := range dataSizes {
		b.Run(ds.name, func(b *testing.B) {
			...
			b.Run("2-tier-promotion", func(b *testing.B) {
				...
				cache, err := daramjwee.New(logger, WithStores(tier0, tier1))
				if err != nil {
					b.Fatalf("Failed to create cache: %v", err)
				}
				defer cache.Close()
				...
				for i := range 1000 {
					...
					tier1.meta[key] = &daramjwee.Metadata{
						ETag:     "promo-etag",
						CachedAt: time.Now(),
					}
				}
			...
			b.Run("3-tier-promotion", func(b *testing.B) {
				...
				for i := range 1000 {
					...
					tier2.meta[key] = &daramjwee.Metadata{
						ETag:     "promo-etag",
						CachedAt: time.Now(),
					}
				}
			...
			b.Run("5-tier-promotion", func(b *testing.B) {
				...
				for i := range 1000 {
					...
					stores[4].(*mockStore).meta[key] = &daramjwee.Metadata{
						ETag:     "promo-etag",
						CachedAt: time.Now(),
					}
				}
			...
			b.Run("2-tier-mixed-workload", func(b *testing.B) {
				...
				for i := range numKeys {
					...
					case 0: // 33% in tier0 (hot hits)
						...
						tier0.meta[key] = &daramjwee.Metadata{
			...
			b.Run("5-tier-mixed-workload", func(b *testing.B) {
				...
				for i := range numKeys {
					...
					if tierIndex < 5 {
						...
						stores[tierIndex].(*mockStore).meta[key] = &daramjwee.Metadata{
							ETag:     fmt.Sprintf("etag-%d", i),
							CachedAt: time.Now(),
						}
					}
			...
			b.Run("set-2-tier", func(b *testing.B) {
				...
				b.RunParallel(func(pb *testing.PB) {
					...
					for pb.Next() {
						...
						writer, err := cache.Set(ctx, key, &daramjwee.Metadata{ETag: "set-etag"})
						if err != nil {
		...
		b.Run(ds.name, func(b *testing.B) {
			...
			b.Run("gc-pressure-2-tier", func(b *testing.B) {
				...
				for i := range numKeys {
					...
					keys[i] = key
					tier1.data[key] = testData
					tier1.meta[key] = &daramjwee.Metadata{ETag: "gc-etag", CachedAt: time.Now()}
				}
			...
			b.Run("gc-pressure-5-tier", func(b *testing.B) {
				...
				for i := range numKeys {
					...
					keys[i] = key
					stores[4].(*mockStore).data[key] = testData
					stores[4].(*mockStore).meta[key] = &daramjwee.Metadata{ETag: "gc-etag", CachedAt: time.Now()}
				}
	...
	b.Run("throughput-2-tier", func(b *testing.B) {
		...
		for i := range numKeys {
			...
			case 0, 1, 2, 3, 4, 5, 6: // 70% in tier0 (hot hits)
				tier0.data[key] = testData
				tier0.meta[key] = &daramjwee.Metadata{ETag: "hot-etag", CachedAt: time.Now()}
			case 7, 8: // 20% in tier1 (promotions)
				tier1.data[key] = testData
				tier1.meta[key] = &daramjwee.Metadata{ETag: "cold-etag", CachedAt: time.Now()}
...
func BenchmarkScalability(b *testing.B) {
	...
	for _, tierCount := range tierCounts {
		b.Run(fmt.Sprintf("Tiers_%d", tierCount), func(b *testing.B) {
			// Create stores
			stores := make([]daramjwee.Store, tierCount)
			...
			for i := range numKeys {
				...
				stores[lastTierIndex].(*mockStore).meta[key] = &daramjwee.Metadata{
					ETag:     "scale-etag",
					CachedAt: time.Now(),
				}
			}
...
// benchmarkFetcher implements daramjwee.Fetcher interface for benchmarking
...
func (f *benchmarkFetcher) Fetch(_ context.Context, _ *daramjwee.Metadata) (*daramjwee.FetchResult, error) {
	f.mu.Lock()
	defer f.mu.Unlock()

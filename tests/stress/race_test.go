...
func TestCache_Chaos_RaceCondition(t *testing.T) {
	...
	for i := 0; i < numKeys; i++ {
		...
		hot.setData(key, "initial-data", &daramjwee.Metadata{})
	}
	...
	for i := 0; i < numGoroutines; i++ {
		go func(goroutineID int) {
			...
			r := rand.daramjwee.New(rand.NewSource(time.Now().UnixNano() + int64(goroutineID)))
			...
			for j := 0; j < 100; j++ {
				...
				case 1: // Set
					writer, err := cache.Set(ctx, key, &daramjwee.Metadata{ETag: "v-set"})

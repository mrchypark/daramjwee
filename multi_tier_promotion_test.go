...
func TestMultiTierPromotion(t *testing.T) {
	...
	t.Run("3-tier promotion from tier 2 to tiers 0 and 1", func(t *testing.T) {
		...
		assert.False(t, fetcher.called, "Fetcher should not be called for tier hit")
	...
	t.Run("2-tier promotion from tier 1 to tier 0", func(t *testing.T) {
		...
		store1.meta["test-key"] = &Metadata{
			ETag:     "test-etag",
			CachedAt: time.Now(),
		}

		cache, err := New(logger, WithStores(store0, store1))
		require.NoError(t, err)
		defer cache.Close()
	...
	t.Run("no promotion needed for primary tier hit", func(t *testing.T) {
		...
		// Verify fetcher was not called
		assert.False(t, fetcher.called, "Fetcher should not be called for primary tier hit")
...
// testFetcher implements Fetcher interface for testing
type testFetcher struct {
	data   string
	called bool
}

func (f *testFetcher) Fetch(ctx context.Context, metadata *Metadata) (*FetchResult, error) {
	f.called = true
	return &FetchResult{

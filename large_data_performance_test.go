...
func measureLargeDataPerformance(t *testing.T, logger log.Logger, size int, useAdaptive bool) time.Duration {
	...
	if useAdaptive {
		options = []Option{
			...
			WithLargeObjectOptimization(256*1024, 1024*1024, 64*1024, 8), // Increased concurrent ops for large data
		}
	} else {
		options = []Option{
			WithHotStore(hotStore),
...
func benchmarkLargeDataOperation(b *testing.B, logger log.Logger, size int, useAdaptive bool) {
	...
	if useAdaptive {
		...
		bufferPool = NewDefaultBufferPoolWithLogger(config, logger)
	}

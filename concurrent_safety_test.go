...
func TestRaceConditionDetection(t *testing.T) {
	...
	testSizes := []int{
		8 * 1024,    // Small
		64 * 1024,   // Medium
		300 * 1024,  // Large (crosses threshold)
		1200 * 1024, // Very Large (crosses threshold)
	}
	...
	for i := 0; i < numGoroutines; i++ {
		...
		go func(goroutineID int) {
			...
			for j := 0; j < operationsPerGoroutine; j++ {
				...
				if localRand.Intn(100) < 5 { // 5% chance
					runtime.Gosched()
				}
			}
		}(i)
...
func TestConcurrentLargeOperations(t *testing.T) {
	...
	failureRate := float64(failed+timeouts) / float64(total) * 100
	t.Logf("Failure rate: %.2f%%", failureRate)
	assert.True(t, failureRate < 80, "Failure rate should be reasonable under concurrency")
}
...
func TestConcurrentMetricsAccuracy(t *testing.T) {
	...
	const operationsPerGoroutine = 400
...
func runConcurrentStressTest(t *testing.T, pool *AdaptiveBufferPool, config ConcurrentTestConfig) ConcurrentTestResult {
	var wg sync.WaitGroup
		...
		go func(goroutineID int) {
			...
			for j := 0; j < config.OperationsPerGoroutine; j++ {
				...
				if len(buf) == size {
					...
					workIntensity := config.StressTestLevel * 10
				...
				if config.StressTestLevel > 5 && localRand.Intn(100) < config.StressTestLevel {
					runtime.Gosched()
				}

				if config.StressTestLevel > 8 && localRand.Intn(1000) < 10 {
...
func validateConcurrentTestResult(t *testing.T, testName string, result ConcurrentTestResult) {
	...
	assert.True(t, result.ThroughputOpsPerSec > 1000, "Throughput should be reasonable (> 1000 ops/sec)")

	// Ensure most operations succeeded
	successRate := float64(result.SuccessfulOps) / float64(result.TotalOperations) * 100

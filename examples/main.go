...
// It introduces a delay to simulate network latency and handles CacheTag-based
// conditional fetching, returning daramjwee.ErrNotModified if the CacheTag matches.
func (f *originFetcher) Fetch(ctx context.Context, oldMetadata *daramjwee.Metadata) (*daramjwee.FetchResult, error) {
	oldETagVal := "none"
	if oldMetadata != nil {
		oldETagVal = oldMetadata.CacheTag
	}
	fmt.Printf("[Origin] Fetching key: %s, (old CacheTag: '%s')", f.key, oldETagVal)
	...
	// If CacheTag matches, indicate that the data has not been modified.
...
func ExampleOriginFetcher_Fetch() {
	...
	// Simulate a conditional fetch with a matching CacheTag
	...
	// Output:
	// [Origin] Fetching key: hello, (old CacheTag: 'none')Fetched data: Hello, Daramjwee! This is the first object.
	//
	// --- Conditional Fetch (Not Modified) ---
	// [Origin] Fetching key: hello, (old CacheTag: 'v1')Data not modified as expected.
}
	...
	http.HandleFunc("/objects/", func(w http.ResponseWriter, r *http.Request) {
		...
		defer func() {
			if err := resp.Close(); err != nil {
				fmt.Printf("[Handler] Error closing response: %v", err)
			}
		}()
		...
		if _, err := io.Copy(w, resp); err != nil {
			fmt.Printf("[Handler] Error copying response to client: %v", err)
		}

...
// s2fifoEntry는 S2-FIFO 정책에서 캐시의 각 아이템을 나타냅니다.
type s2fifoEntry struct {
	key    string
	size   int64
...
// NewS2FIFOPolicy는 새로운 S2-FIFO 정책을 생성합니다.
// totalCapacity는 캐시의 총 용량(바이트)이며,
// smallRatio는 전체 용량 중 smallQueue에 할당할 비율입니다 (예: 0.1은 10%).
func NewS2FIFOPolicy(totalCapacity int64, smallRatio float64) daramjwee.EvictionPolicy {
...
var _ daramjwee.EvictionPolicy = (*S2FIFOPolicy)(nil)

// Add는 새로운 아이템을 캐시에 추가합니다. 아이템은 항상 smallQueue로 먼저 들어갑니다.
func (p *S2FIFOPolicy) Add(key string, size int64) {
	if elem, ok := p.cache[key]; ok {
		// 이미 존재하는 아이템이면 사이즈만 업데이트합니다.
		entry := elem.Value.(*s2fifoEntry)
		if entry.isMain {
	...
	// 새 아이템 추가
	entry := &s2fifoEntry{key: key, size: size, isMain: false, wasHit: false}
...
func (p *S2FIFOPolicy) Touch(key string) {
	elem, ok := p.cache[key]
	if !ok {
		return
	}
...
func (p *S2FIFOPolicy) Remove(key string) {
	if elem, ok := p.cache[key]; ok {
...
func (p *S2FIFOPolicy) Evict() []string {
...
func (p *S2FIFOPolicy) removeElement(e *list.Element) *s2fifoEntry {
	entry := e.Value.(*s2fifoEntry)
	if entry.isMain {

...
func TestTierError(t *testing.T) {
	t.Run("TierError creation and formatting", func(t *testing.T) {
	...
	t.Run("TierError without key", func(t *testing.T) {
	...
	t.Run("TierError unwrapping", func(t *testing.T) {
	...
	t.Run("TierError with custom error type", func(t *testing.T) {
...
func TestTierErrorInCacheOperations(t *testing.T) {
	t.Run("Get operation with tier errors", func(t *testing.T) {
		// Create a failing store for tier 1
		store0 := newMockStore()
		store1 := &failingStore{err: errors.New("network timeout")}
		store2 := newMockStore()

		cache, err := New(nil, WithStores(store0, store1, store2))
	...
	t.Run("Delete operation with tier errors", func(t *testing.T) {
		...
		store0 := newMockStore()
		store1 := &failingStore{err: errors.New("permission denied")}
		store2 := newMockStore()
		...
		// Should return TierError from the failing store
		require.Error(t, err)
	...
	t.Run("Promotion operation with tier errors", func(t *testing.T) {
		...
		store0 := &failingStore{err: errors.New("disk full")}
		store1 := newMockStore()
		store2 := newMockStore()

		// Put data only in store2
		store2.data["test-key"] = []byte("tier 2 data")
		store2.meta["test-key"] = &Metadata{ETag: "test-etag"}
...
func (f *failingStore) GetStream(ctx context.Context, key string) (io.ReadCloser, *Metadata, error) {
	return nil, nil, f.err
}

func (f *failingStore) SetWithWriter(ctx context.Context, key string, metadata *Metadata) (io.WriteCloser, error) {
	return nil, f.err
}
...
func (f *failingStore) Stat(ctx context.Context, key string) (*Metadata, error) {
	return nil, f.err
}
...
// simpleFetcher implements Fetcher interface for testing
type simpleFetcher struct {
	data   string
	called bool
}

func (f *simpleFetcher) Fetch(ctx context.Context, metadata *Metadata) (*FetchResult, error) {
	f.called = true
	return &FetchResult{
		Body:     io.NopCloser(strings.NewReader(f.data)),
		Metadata: &Metadata{ETag: "origin-etag"},
	}, nil
}
